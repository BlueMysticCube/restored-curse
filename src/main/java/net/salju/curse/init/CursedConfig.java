...
public class CursedConfig {
    ...
    static {
        ...
        ATTACK_CHANCE = BUILDER.comment("The chance per tick, that mobs around a cursed player get angry").defineInRange("Attack Chance", 0.025,0, 1);
        ...
        TARGET_MIGRATION_CHANCE = BUILDER.comment("The chance per tick, that mobs around a cursed player change their target to the cursed soul").defineInRange("Target Migration Chance", 0.5, 0, 1);
        TARGET_MIGRATION_COUNT = BUILDER.comment("How many mobs change their target to a cursed player at once, as a proportion of all mobs in the radius (~28m)").defineInRange("Target Migration Count", 0.1, 0, 1);
